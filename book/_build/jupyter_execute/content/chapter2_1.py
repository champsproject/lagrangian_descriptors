...
Normally hyperbolic invariant sets play a particularly important role in phase space transport, particularly when the invariant set has the additional property of being an invariant manifold. For our purposes the reader can think of an invariant manifold as being a generalization of a curve or surface for which  the equation defining such a mathematical object is differentiable \cite{guillemin2010differential}.  {\em Normally hyperbolic invariant manifolds} (NHIMs) have invariant stable and unstable manifolds that form the skeleton or framework,  on which phase space transport  occurs. But before we discuss normally hyperbolic invariant manifolds in general, we discuss the simplest type of non-trivial type of normally hyperbolic invariant manifold—a hyperbolic periodic orbit.
...
In two degree-of-freedom Hamiltonian systems a one dimensional unstable periodic orbits has two dimensional stable and unstable manifolds that can ``divide’’ the energy surface and can be used to create invariant regions (``lobes’’)  that transport throughout the phase space, while respecting the ``barrier nature’’ of invariant manifolds.
...
NHIMs are a generalization of unstable periodic orbits to more than two degrees-of-freedom in this sense that they {\em may} have stable and unstable manifolds of codimension one in the energy surface and, hence, for the framework on which phase space transport occurs  in Hamiltonian systems with more than two degrees-of-freedom \cite{wiggins1990geometry}.  Now we want to develop this idea of a NHIM, and its consequences, in more detail. Recent references for this topic are \cite{Wigginsbook1994,Wiggins2001,Wigginsbook2004,Eldering2013}, but we will begin with an elementary description.


In understanding what is meant by ``normally hyperbolic invariant manifold’’ we start by explaining the last word in the phrase and working towards the first.

\begin{itemize}

\item A {\em manifold} is the natural generalization of a differentiable curve or surface.

\item A set in the constant energy level set is called {\em invariant} under the flow if any trajectory in the set is contained in the set at all times (link to definition in previous chapter).

\item {\em Normal hyperbolicity} means that locally under the linearized dynamics, the growth and decay rates in the normal directions to the manifold dominate the growth and decay rates tangent to it. These growth rates are quantified by eigenvalues of the matrix associated with the linearization for equilibria, Floquet exponents for the monodromy matrix associated with periodic orbits, and generalized Lyapunov exponents for more general invariant manifolds \cite{Fenichel1971}.
